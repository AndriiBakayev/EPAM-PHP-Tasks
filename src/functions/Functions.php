...
/**
 * Functions Class of functions which says Hello :)  and functions that demonstrates the func_get_args() function
 */
class Functions
{
    /**
     ...
     * 
     * Feature: with bool value true it returns "Hello 1", false or null returns "Hello "
     ...
     *
     * @param $arg argument that verifyed to be a number, string or bool and call the sayHelloArgument with it
     * 
    ...
    public function sayHelloArgumentWrapper($arg): string
    {
        ...
        if (!is_scalar($arg) && !is_null($arg)) {
            throw new \InvalidArgumentException('sayHelloArgument() function accepts only scalar types: number, string or bool. ');
            return "Hello";//
        }
    ...
    public function countArgumentsWrapper(): array
    {
        foreach (func_get_args() as $val) {
            if (!is_string($val)) {
                throw new \InvalidArgumentException('countArgumentsWrapper() function accepts strings only');
            }
        }
